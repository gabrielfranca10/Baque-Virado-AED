...
int DesenharBotao(const char *texto, int y, int selecionado) {
    ...
    if (selecionado || hover) {
        ...
        DrawText(texto, x + (larguraBotao - tw) / 2, y + (alturaBotao - 26) / 2, 26, COR_FUNDO);
    } else {
        ...
        DrawText(texto, x + (larguraBotao - tw) / 2, y + (alturaBotao - 24) / 2, 24, COR_BRANCO);
    }
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (tela == TELA_MENU) {
            ...
            // Fundo: imagem redimensionada para a tela
            if (fundo.id > 0) {
            ...
            for (int i = 0; i < TOTAL_OPCOES; i++) {
                ...
                int clicado = DesenharBotao(rotulos[i], y, selecao == i);
                if (clicado) {
