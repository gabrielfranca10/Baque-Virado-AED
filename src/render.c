...
const int          KEYS_JOGO[NUM_COLUNAS]   = { KEY_D, KEY_F, KEY_J, KEY_K };
...
void DesenharJogoEncerrado(int pontuacao) {
    ...
    DesenharTextoCentralizado("R — jogar a fase 3 novamente", 434, 15, COR_CINZA);
}
    ...
    if (n == 0) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (ranking[i].nome[0])
                ...
                nome = "---";
            DrawText(nome, 190, yInicio + i*passo, 22, cor);
...
void DesenharContagem(int fase, float tempoRestante, Texture2D fundo_jogo) {
    ...
    DrawRectangle(0, 0, sw, sh, (Color){10, 5, 25, 210});

    DesenharTextoCentralizado(FASES[fase].titulo, 150, 22, COR_CINZA);
    DesenharTextoCentralizado("Prepare-se!", 186, 30, COR_BRANCO);
    DesenharTextoCentralizado(FASES[fase].artista, 435, 14, COR_CINZA);

    const char *txt;
    Color       cor;
    if      (tempoRestante > 3.0f) { txt = "3";    cor = COR_VERMELHO; }
    else if (tempoRestante > 2.0f) { txt = "2";    cor = COR_DOURADO; }
    else if (tempoRestante > 1.0f) { txt = "1";    cor = (Color){60, 200, 110, 255}; }
    else                           { txt = "VAI!"; cor = COR_DOURADO; }

    float frac = 1.0f - fmodf(tempoRestante, 1.0f);
    int   tam;
    if (tempoRestante <= 1.0f)
        tam = 76;
    else
        tam = (int)(76.0f + 22.0f * frac);
    int tw = MeasureText(txt, tam);
    DrawText(txt, (sw - tw) / 2, sh / 2 - tam / 2 + 30, tam, cor);
}
...
void DesenharEntradaNome(int pontuacao, const char *nomeInput, float tempo) {
    ...
    DesenharLinhaDourada(55, 40);
    DesenharTextoCentralizado("PARABENS!", 68, 44, COR_DOURADO);
    DesenharLinhaDourada(122, 40);

    char buf[32];
    snprintf(buf, sizeof(buf), "%d PONTOS", pontuacao);
    DesenharTextoCentralizado(buf, 140, 28, COR_DOURADO);
    DesenharTextoCentralizado("Digite seu nome para o ranking:", 192, 20, COR_BRANCO);

    int bw = 400, bh = 54;
    int bx = (sw - bw) / 2;
    Rectangle campo = {bx, 234, bw, bh};
    DrawRectangleRounded(campo, 0.2f, 8, (Color){20, 15, 40, 220});
    DrawRectangleRoundedLines(campo, 0.2f, 8, COR_DOURADO);

    char exibir[MAX_NOME + 2];
    int cursor = (int)(tempo * 2) % 2;
    if (cursor)
        snprintf(exibir, sizeof(exibir), "%s|", nomeInput);
    else
        snprintf(exibir, sizeof(exibir), "%s ", nomeInput);
    int tw = MeasureText(exibir, 28);
    DrawText(exibir, bx + (bw - tw) / 2, 234 + (bh - 28) / 2, 28, COR_BRANCO);

    DesenharLinhaDourada(306, 60);
    DesenharTextoCentralizado("ENTER — confirmar",           320, 15, COR_DOURADO);
    DesenharTextoCentralizado("ESC — pular e voltar ao menu", 342, 15, COR_CINZA);
}
